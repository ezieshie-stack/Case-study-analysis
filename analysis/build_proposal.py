...
def build():
    ...
    S.append(p("Each shift counted once, classified by its final cancellation event. Source: models workbook, tab 3.", cap))
    S.append(p(
    ...
    S.append(p("Source: models workbook, tab 5.", cap))
    S.append(p(
    ...
    S.append(p("Supporting analysis. All tables are generated by the attached, fully reproducible Excel "
               "models (Clipboard_Analysis_Models.xlsx); tab references given per table.", sub_st))
    ...
    S.append(tbl([
        ...
        ["Cancel events on clean universe (early / late / NCNS)", "3,347 / 2,436 / 1,177", "2"],
        ["Notice given, worker cancels: ≥72h / 24–72h / 4–24h / &lt;4h", "48.8% / 9.1% / 18.7% / 23.4%", "2b"],
        ["Cancel-driven empty shifts (late / NCNS / early)", "1,376 / 976 / 282 = 2,634", "4"],
        ["Facility charge destroyed by late+NCNS empties (4 mo)", "$729,559 gross; ~$160K CBH take", "10"],
        ["Facilities hit by ≥1 late/NCNS empty; top-10 share", "55 of 66; 50.2%", "11"],
        ["Late+NCNS shift-failures per week", "~198", "11"],
    ], [3.0 * inch, 2.2 * inch, 1.0 * inch]))
    ...
    S.append(p("Workbook tab 8. The 3+ flag covers 16.3% of bookings and catches 29.5% of failures / 27.1% of NCNS.", cap))
    S.append(tbl([
    ...
    S.append(p(
        "Workbook tab 6. The naive view brands last-minute claimers high-risk; the corrected view shows they are the most "
        "reliable segment. 354 of 1,306 last-minute claims (27%) were rescues of already-cancelled shifts; rescued "
        "shifts were worked 80.5% of the time (tab 7).", cap))
    S.append(p(
        "First-timer wall: of 351 bookings ending in NCNS, 42.2% came from professionals with zero prior "
        "offenses and 69.8% from professionals with zero prior NCNS. Detection lag: median NCNS event is "
        "logged 34.8h after shift start (tab 9)."))
